...
rouge= (250,0,0)
orange= (255, 120, 50)
bord1,bord2=1000,900 #taille de la fenêtre
...
sortiex1h=bord1*90/100
sortiex2h=bord1*90/100
sortiey1h=bord2*40/100
sortiey2h=bord2
sortiexh=abs(sortiex1h+sortiex2h)/2
sortieyh=abs(sortiey1h+sortiey2h)/2

sortiex1b=bord1*90/100
sortiex2b=bord1*90/100
sortiey1b=bord2*60/100
sortiey2b=0
sortiexb=abs(sortiex1b+sortiex2b)/2
sortieyb=abs(sortiey1b+sortiey2b)/2
...
#obstacles
porte_haut = [(bord1,0), (bord1*80/100, 0), (bord1*80/100, bord2*18/100),(bord1,bord2*18/100)]
porte_bas = [(bord1,bord2), (bord1*80/100,bord2), (bord1*80/100, bord2*82/100),(bord1,bord2*82/100)]
porte_milieu = [(bord1,bord2*68/100), (bord1*80/100,bord2*68/100), (bord1*80/100, bord2*32/100),(bord1,bord2*32/100)]
poteau1 = (sortiex-150,sortiey)
poteau2 = (sortiex-200,sortieyh+50)
poteau3 = (sortiex-200,sortieyb-80)
rayon_poteau1 = 20
rayon_poteau2 = 20
rayon_poteau3 = 20
panneau2 = [ (sortiex1*60/100,sortiey*130/100), (sortiex1*70/100,sortiey*130/100), (sortiex1*70/100,sortiey*60/100), (sortiex1*60/100,sortiey*60/100) ]

panneau = [ (sortiex1*60/100,bord2*75/100), (sortiex1*70/100,bord2*75/100), (sortiex1*70/100,bord2*25/100), (sortiex1*60/100,bord2*25/100) ]


### Fonctions

def directionh(x,y):
    d=(((sortiexh-x)**2+(sortieyh - y)**2)**(1/2)) #distance avec la sortie
    dx, dy = (sortiexh- x)/d, (sortieyh - y)/d+(y-bord2/2)/(bord2*1.5)
    return (dx,dy)

def directionb(x,y):
    d=(((sortiexb-x)**2+(sortieyb - y)**2)**(1/2)) #distance avec la sortie
    dx, dy = (sortiexb- x)/d, (sortieyb - y)/d+(y-bord2/2)/(bord2*1.5)
    return(dx,dy)

def direction_fond(x,y):
    d=(((bord1-x)**2+(sortiey - y)**2)**(1/2)) #distance avec la sortie
    dx, dy = (bord1- x)/d, (sortiey - y)/d+(y-bord2/2)/bord2
    return(dx,dy)
...
#réaction aux différents obstacles (on peut les choisir en les prenant en compte dans la fonction move et en les affichant ou non)
    ...
    def react_poteau1(self):
        ...
        if (distp < rayon_poteau1 + rayon + 10):
            if self.y>bord2/2:
                dx,dy=directionh(self.x,self.y)
                self.x -= dx
                self.x += 0.6*1.5
                ...
                dx,dy=directionb(self.x,self.y)
                self.x -= dx
                self.x += 0.6*1.5
    ...
    def entree(self):
		if self.x >= bord1*78/100 and self.y >= 0 and self.y < bord2*22/100:
		    dx,dy=directionh(self.x,self.y)
		    self.y-=dy
		    self.vx = V
		elif self.x >= bord1*78/100 and self.y >= bord2*77/100 and self.y < bord2:
		    dx,dy=directionb(self.x,self.y)
		    self.vy-=dy
		    self.vx = V
        else:
            if self.x >= bord1*78/100:
                self.vy=0
                self.vx = V

    def entree2(self):
        if self.x >= bord1*70/100:
            self.vy=0
            self.vx = V
...
while continuer == True:
    ...
    py.draw.polygon(fenetre, (0, 0, 0), porte_milieu)
    ...
    #on peut figer les humains
        ...
        for humain in humains:
            (dx,dy)=directionh(humain.x,humain.y)
            humain.x-=dx
            humain.y-=dy
